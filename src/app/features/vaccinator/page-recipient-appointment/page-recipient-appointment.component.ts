...
export class PageRecipientAppointmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    localStorage.setItem('qrscan','true');
  }

  back(): void {
    this._router.navigate(['receptionist', 'walk-in']);
  }

  bookAppointment(): void {
    this._router.navigate(['receptionist', 'book-appointment', this.recipientId]);
  }

  adverseEventReport(visitId): void {
    this._router.navigate(['vaccinator', 'advert-event', visitId,'card']);
  }
  ...
  private loadRecipient(): void {
    ...
    this._siteAdminService.searchRecipients(`id=${this.recipientId}`, 1, 1).subscribe((result) => {
      ...
      //localStorage.setItem('recipientInQRScan',JSON.stringify(this.recipient));
    })
