...
export class PageEditProfileComponent implements OnInit {
  ...
  buildForm() {
    this.authAdminEditForm = this._formBuilder.group({
  ...
  setFormControlValue() {
    this.currentAdminProfile = JSON.parse(
      localStorage.getItem(this.currentAuthLoggedKey + '-user')
    );

    this.currentAdminProfile = this.currentAdminProfile.hasOwnProperty('user_profile') ? this.currentAdminProfile.user_profile : '';

    const answers = this.currentAdminProfile.static_questionnaire_answers;
    if (!!answers && answers.length > 0) {
      var Insurance = answers.find(
        (x) => x.question_text.trim() === 'Insurance'
      );
      var insuranceprovidername = answers.find(
        (x) => x.question_text.trim() === 'Insurance Provider Name'
      );
      var medicalGroupNumber = answers.find(
        (x) => x.question_text.trim() === 'Medical Group Number'
      );
      var memberId = answers.find(
        (x) => x.question_text.trim() === 'Medical ID'
      );
    }

    this.temp = this.currentAdminProfile.mobile_number.substring(
      0,
      this.currentAdminProfile.mobile_number.length - 10
    );
    if(this.currentAdminProfile.mobile_number==null){
      this.currentAdminProfile.mobile_number=''
    } else{
      this.currentAdminProfile.mobile_number= this.currentAdminProfile.mobile_number.replace(this.temp,"")
    }
    this.authAdminEditForm.patchValue({
      firstName: this.currentAdminProfile.fname
        ? this.currentAdminProfile.fname
        : '',
      lastName: this.currentAdminProfile.lname
        ? this.currentAdminProfile.lname
        : '',
      email: this.currentAdminProfile.email
        ? this.currentAdminProfile.email
        : '',
      username: this.currentAdminProfile.user_name
        ? this.currentAdminProfile.user_name
        : '',
      phoneNumber: this.currentAdminProfile.mobile_number
        ? this.currentAdminProfile.mobile_number
        : '',
      gender: this.currentAdminProfile.gender
        ? this.titleCaseWord(this.currentAdminProfile.gender)
        : '',
      dob: this.currentAdminProfile.dob
        ? new Date(this.currentAdminProfile.dob?.replace(/-/g, '\/'))
        : '',
      address1: this.currentAdminProfile.address1
        ? this.currentAdminProfile.address1
        : '',
      address2: this.currentAdminProfile.address2
        ? this.currentAdminProfile.address2
        : '',
      address3: this.currentAdminProfile.address3
        ? this.currentAdminProfile.address3
        : '',
      zip: this.currentAdminProfile.zip ? this.currentAdminProfile.zip : '',
      city: this.currentAdminProfile.city ? this.currentAdminProfile.city : '',
      state: this.currentAdminProfile.state
        ? this.currentAdminProfile.state
        : '',
      country: this.currentAdminProfile.country
        ? this.currentAdminProfile.country
        : 'United States',
      county: this.currentAdminProfile.county
        ? this.currentAdminProfile.county
        : '',
      insurance: Insurance ? Insurance.answers[0] : '',
      insuranceprovidername: insuranceprovidername
        ? insuranceprovidername.answers[0]
        : '',
      medicalGroupNumber: medicalGroupNumber
        ? medicalGroupNumber.answers[0]
        : '',
      memberId: memberId ? memberId.answers[0] : '',
    });
  }

  authAdminEditFormSub() {
    if (!this.authAdminEditForm.valid) {
      return;
    }
    ...
    let formData = this.authAdminEditForm.value;
    this.currentAdminProfile.fname = formData.firstName;
    this.currentAdminProfile.lname = formData.lastName;
    this.currentAdminProfile.user_profile.fname = formData.firstName;
    this.currentAdminProfile.user_profile.lname = formData.lastName;
    this.currentAdminProfile.user_profile.email = formData.email;
    this.currentAdminProfile.user_profile.user_name = formData.username;
    this.currentAdminProfile.user_profile.gender = formData.gender;
    (this.currentAdminProfile.user_profile.dob = formData.dob
      ? this.datepipe.transform(formData.dob, 'yyyy-MM-dd')
      : ''),
      (this.currentAdminProfile.user_profile.address1 = formData.address1);
    this.currentAdminProfile.user_profile.address2 = formData.address2;
    this.currentAdminProfile.user_profile.address3 = formData.address3;
    this.currentAdminProfile.user_profile.zip = formData.zip;
    this.currentAdminProfile.user_profile.city = formData.city;
    this.currentAdminProfile.user_profile.state = formData.state;
    this.currentAdminProfile.user_profile.country = formData.country;
    this.currentAdminProfile.user_profile.county = formData.county;


    console.log('profieData',this.currentAdminProfile.user_profile)
    this.adminService
      .saveAdminProfile(this.currentAdminProfile.user_profile)
      .subscribe(
        (res) => {
          this._router.navigate(['/auth/admin-my-profile']);
        },
        (err) => {
          console.log(err);
        }
  ...
  checkUserByEmailID(emailControl: FormControl): Observable<any> {
    ...
    return this.adminService.userCheck(emailId).pipe(
      map((existingAdmin) => {
        if (existingAdmin) {
          return { existingAdmin: true };
        } else {
          return null;
        }
      })
  ...
  onRaceCheckboxChange(value: string, isChecked: boolean) {
    const raceFormArray = <FormArray>(
      (<FormGroup>this.authAdminEditForm.controls.answers).controls
        .raceFormArray
    );
    if (isChecked) {
  ...
  fetcheditzipdetails(zipvalue) {
    ...
    this.adminService.fetchZipDetails(zip).subscribe(
      (res) => {
        this.authAdminEditForm.controls.city.setValue(res.city);
        this.authAdminEditForm.controls.state.setValue(res.state);
        this.authAdminEditForm.controls.country.setValue(res.country);
        this.show = true;
  ...
  getError(){
    this.authAdminEditForm.controls.city.setValue('');
    this.authAdminEditForm.controls.state.setValue('');
    this.show = false;
    this.stateshow = true;
  }
}
